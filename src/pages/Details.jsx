  ...
  let suggestions = movies.filter((data)=>{
   ...
   function volumeHandler(){
    ...
    return(
      ...
      <ReactPlayer width="100vw" className="scale-[1.50]"  loop='true'  muted={volume} playing="true" height="80vh" url={movie.youtube_trailer} />
      
       <div className="absolute  w-[400px] bottom-20 left-24 text-center">
       <div className="bg-orange-700 inline-block text-white px-4 py-1 rounded text-xs">IMDB Ratings {movie.imdb_rating}</div>
         <h1 className='text-white font-semibold mt-3 '>{movie.title}</h1>
         <p className='text-white font-sm mt-3'>{movie.description}</p>
         <button className='p-3 m-auto bg-black text-white rounded-lg mt-3'>Add to Watchlist</button>
       </div>
